...
*/
function viewStudentStats(student_status, bool){
	var visible = "hidden";
	if (!bool){
			document.getElementById(student_status).style.visibility = visible;
			document.getElementById(student_status).style.height = "0pt";
	}
	else{
		visible = "visible";
		document.getElementById(student_status).style.visibility = visible;
...
function loadPlayersPage(){
	...
	for(var i = 0; i < players.length; i++){
		name_list += '<a class="dropdown-item" href="#" onclick="switchPlayers(' + i + ')">' + players[i].name + '</a>';
	}
	document.getElementById('player_selector').innerHTML = name_list;
}
function switchPlayers(i){
	var avgrec = players[i].receiving_yards/players[i].games_played;
	avgrec = avgrec.toFixed(0);
	var avgpass = players[i].pass_yards/players[i].games_played;
	avgpass = avgpass.toFixed(0);
	var avgrush = players[i].rushing_yards/players[i].games_played;
	avgrush = avgrush.toFixed(0);
